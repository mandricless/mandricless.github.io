...
function ChoiceButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`group rounded-[24px] border px-4 py-3 text-left transition-all duration-300 ${
        active
          ? "border-white/50 bg-white/26 text-slate-950 shadow-[0_10px_30px_rgba(255,255,255,0.18)] backdrop-blur-2xl"
          : "border-white/30 bg-white/12 text-slate-900 backdrop-blur-xl hover:bg-white/18 hover:border-white/45"
      }`}
    >
      {children}
    </button>
            ...
            <CardContent className="relative p-8 md:p-10">
              ...
              <div className="relative">
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  ...
                  <Button
                    variant="outline"
                    className="rounded-[22px] border-white/40 bg-white/18 text-slate-950 backdrop-blur-xl hover:bg-white/26"
                    onClick={resetAll}
                  >
          ...
          <div className="space-y-6">
            <Card className={glassCard}>
              <CardHeader>
                ...
                <CardDescription className="text-slate-900/75">Если не хочется выбирать всё вручную, можно начать с готового сценария</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  {presets.map((preset) => (
                    <button
                      ...
                      className="rounded-[24px] border border-white/30 bg-white/14 p-4 text-left backdrop-blur-xl transition-all hover:bg-white/20 hover:border-white/45"
                    >
